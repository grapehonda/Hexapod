...
void ControlInput(void) {
  ...
  Serial.print("Wireless avail: "); Serial.println(Serial3.available()); // Debug
  if (Serial3.available() > 20) {  // Lowered threshold
    ...
    Serial.print("Wireless bytes: "); Serial.println(bytesRead); // Debug
    Serial.print("Wireless buf: "); Serial.println(wirelessBuf); // Debug
    if (bytesRead >= 38) {  // Min for 19 vals (~38 chars: "0,0,...\n")
      // Trim whitespace
      char* ptr = wirelessBuf;
      while (*ptr == ' ' || *ptr == '\r' || *ptr == '\n') ptr++;
      int commaCount = countCommas(ptr);
      Serial.print("Wireless commas: "); Serial.println(commaCount); // Debug
      if (commaCount == 18 && parseBufferToValues(ptr, values)) {
        Serial.println("Wireless parse OK"); // Debug
        byte switch_val = values[9];  // ch10: piControllerSwitch
        Serial.print("Switch val: "); Serial.println(switch_val); // Debug
        if (switch_val > 127) {  // >127 for high (was 128, now >127 for tolerance)
          // Use wireless
          gotInput = true;
        } else {
          // Switch low, try Pi
          Serial.print("Pi avail: "); Serial.println(Serial2.available()); // Debug
          if (Serial2.available() > 20) {
            char piBuf[150] = {0};
            int piBytes = Serial2.readBytesUntil('\n', piBuf, sizeof(piBuf) - 1);
            Serial.print("Pi bytes: "); Serial.println(piBytes); // Debug
            Serial.print("Pi buf: "); Serial.println(piBuf); // Debug
            if (piBytes >= 38) {  // Added garbage check
              char* piPtr = piBuf;
              while (*piPtr == ' ' || *piPtr == '\r' || *piPtr == '\n') piPtr++;
              int piCommaCount = countCommas(piPtr);
              Serial.print("Pi commas: "); Serial.println(piCommaCount); // Debug
              if (piCommaCount == 18 && parseBufferToValues(piPtr, values)) {
                Serial.println("Pi parse OK"); // Debug
                gotInput = true;
              } else {
                Serial.println("Pi garbage data - ignoring"); // Debug
              }
            } else {
              Serial.println("Pi short data - ignoring"); // Debug
            }
          } else {
            Serial.println("No Pi data"); // Debug
          }
        }
      } else {
        Serial.println("Wireless parse fail"); // Debug
      }
    } else {
      Serial.println("Wireless short data - ignoring"); // Debug
    }
  } else {
  ...
  if (gotInput) {
    Serial.println("Got input OK"); // Debug
  } else {
    Serial.println("No valid input"); // Debug
