...
// Toggle switches (6, LOW = on = 255, wire to GND + pin, INPUT_PULLUP):
//   D2: HexOn
...
// QGVA 2.2 TFT SPI 240x320 LCD (ILI9341 or similar):
//   TFT VCC → Mega 5V
//   TFT GND → Mega GND
//   TFT CS → Mega D53 (SS)
//   TFT RESET → Mega D48
//   TFT DC → Mega D47
//   TFT SDI (MOSI) → Mega D51 (MOSI)
//   TFT SCK → Mega D52 (SCK)
//   TFT LED → Mega 5V (or PWM pin for brightness, e.g., D46)
//   TFT SDO (MISO) → Mega D50 (MISO, optional)

// SD Card (shared SPI):
//   Note: For modules like HiLetgo ILI9341 with built-in SD slot, the SD pins are often broken out separately on the board.
//   You need to wire them to share the SPI bus with the TFT:
//   SD CS → Mega D49
//   SD MOSI → Mega D51 (MOSI) or bridge to TFT SDI/MOSI pin
//   SD MISO → Mega D50 (MISO) or bridge to TFT SDO/MISO pin
//   SD SCK → Mega D52 (SCK) or bridge to TFT SCK pin
//   If the SD pins are not wired to the SPI bus, SD initialization will fail.
...
// Adafruit_ILI9341 setup
Adafruit_ILI9341 tft = Adafruit_ILI9341(TFT_CS, TFT_DC, TFT_RST);

// Rotary encoder setup
Rotary encoder = Rotary(encoderCLK, encoderDT);
volatile int deltaAccum = 0;  // Accumulate changes from ISR
...
// Globals - check if these names match up with Input_SBUS.h, channel order is now different, but needs to use same labels.
byte g_fHexOn = 0;
